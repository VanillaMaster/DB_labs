//const { resolve } = require("@tauri-apps/api/path");
...
await (async function() {  
    ...
    let data = JSON.parse(await window.__TAURI__.invoke("retrieve_data",{name:"books",filter:(searchFilter || ""),max:`${maxItemCount}`,skip:((currentPage - 1) * maxItemCount)}))
    //console.log(data);
    let fragment = document.createDocumentFragment();
...
function createTBodyFragment(data){
    ...
    data.rows.forEach(element => {
        ...
        let td = tableActions.cloneNode(true);
        let removeBtn = td.querySelector(".delete");
        const id = element.id;
        removeBtn.addEventListener("click",async ()=>{
            console.log(id);
            //await globalThis.DB.deleteData("books",id);
            await window.__TAURI__.invoke("delete_data",{name:"books",id:id});
            updateTable();
        });
...
const selectDefaultValue = (function(){
    let fragment = document.createDocumentFragment();
...
addBtn.addEventListener("click",async()=>{
    ...
    let result = await Promise.all([
        window.__TAURI__.invoke("retrieve_data",{name:"authors",filter:"",max:"all",skip:0}),
        window.__TAURI__.invoke("retrieve_data",{name:"publishers",filter:"",max:"all",skip:0}),
    ]);

    for (let i = 0; i < result.length; i++) {result[i] = JSON.parse(result[i]);}

    //authors
    while (insertAuthorId.firstChild)
        insertAuthorId.removeChild(insertAuthorId.lastChild);
    insertAuthorId.append(selectDefaultValue.cloneNode(true));
    ...
    insertPublisherId.append(selectDefaultValue.cloneNode(true));
...
modalInsert.addEventListener('click',()=>{
    ...
    if (isValid) {
        ...
        (async function(){
            ...
            await window.__TAURI__.invoke("insert_data",{name:"books",value:(values.join())});
            modal.close();
            updateTable();
        })();
    } else {
